...
void UInv_InventoryGrid::ChangeHoverType(const int32 Index, const FIntPoint& Dimensions, EInv_GridSlotState HoverType)
{
	UnhighlightSlots(LastHighlightedIndex, LastHighlightedDimensions);

	UInv_InventoryStatics::ForEach2D(GridSlots, Index, Dimensions, Columns, [HoverType](UInv_GridSlot* GridSlot)
	{
		switch (HoverType)
		{
...
FInv_SlotAvailabilityResult UInv_InventoryGrid::HasRoomForItem(const FInv_ItemManifest& Manifest, const int32 StackAmountOverride)
{
	...
	for (const auto& GridSlot : GridSlots)
	{
		...
		Result.RemainedItems = AmountToFill;
	...
	TSet<int32>& OutTentativelyClaimed, const FGameplayTag& ItemType, const int32 MaxStackSize)
{
	...
	{
		if (CheckSlotConstraints(CheckedIndices, OutTentativelyClaimed, GridSlot, SubGridSlot, ItemType, MaxStackSize))
		{
...
bool UInv_InventoryGrid::CheckSlotConstraints(const TSet<int32>& CheckedIndices, TSet<int32>& OutTentativelyClaimed, const UInv_GridSlot* GridSlot,
	const UInv_GridSlot* SubGridSlot, const FGameplayTag& ItemType, const int32 MaxStackSize) const
{
...
void UInv_InventoryGrid::SwapStackCounts(const int32 ClickedItemStackCount, const int32 HoveredItemStackCount,
	const int32 Index)
{
	UInv_GridSlot* GridSlot = GridSlots[Index];
	GridSlot->SetStackCount(HoveredItemStackCount);

	UInv_SlottedItem* ClickedSlottedItem = SlottedItems.FindChecked(Index);
	ClickedSlottedItem->UpdateStackCount(HoveredItemStackCount);
...
void UInv_InventoryGrid::ConsumeHoverItemStacks(const int32 ClickedItemStackCount, const int32 HoveredItemStackCount,
	const int32 Index)
{
	const int32 AmountToTransfer = HoveredItemStackCount;
	const int32 NewClickedStackCount = ClickedItemStackCount + AmountToTransfer;
